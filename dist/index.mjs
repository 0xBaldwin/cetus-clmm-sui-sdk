...
var _TransactionUtil = class {
  ...
  static buildAddLiquidityFixTokenArgs(tx, sdk, allCoinAsset, params, primaryCoinAInputs, primaryCoinBInputs) {
    ...
    if (sdk.packageOveride) {
      tx.moveCall({
        target: `${sdk.packageOveride}::cetus::${functionName}`,
        typeArguments,
        arguments: args
      });
    } else {
...
var PositionModule = class {
  ...
  async removeLiquidityTransactionPayload(params, tx) {
    ...
    if (this.sdk.packageOveride) {
      tx.moveCall({
        target: `${this.sdk.packageOveride}::cetus::${functionName}`,
        typeArguments,
        arguments: args
      });
    } else {
  ...
  async closePositionTransactionPayload(params, tx) {
    ...
    if (this.sdk.packageOveride) {
      tx.moveCall({
        target: `${this.sdk.packageOveride}::cetus::close_position`,
        typeArguments,
        arguments: [
          tx.object(getPackagerConfigs(clmm_pool).global_config_id),
          tx.object(params.pool_id),
...
var CetusClmmSDK = class {
   ...
   */
  get packageOveride() {
    return this._packageOveride;
  }
  /**
   * Setter for the sender address property.
   * @param {string} value - The new sender address value.
   */
  set packageOveride(value) {
    this._packageOveride = value;
  }
  /**
...
function initMainnetSDK(fullNodeUrl, wallet, packageOveride) {
  if (fullNodeUrl) {
  ...
  if (packageOveride) {
    sdk.packageOveride = packageOveride;
  }
  return sdk;
}
...
function initCetusSDK(options) {
  const { network, fullNodeUrl, wallet, packageOveride } = options;
  return network === "mainnet" ? initMainnetSDK(fullNodeUrl, wallet, packageOveride) : initTestnetSDK(fullNodeUrl, wallet);
}
